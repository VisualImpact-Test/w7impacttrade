...
class M_storecheck extends MY_Model
{
	...
	public function getInfoPuntoDeVenta($post)
	{
		$sql = "
			...
			SELECT DISTINCT 
				...
				c.direccion
			FROM trade.cliente c
				INNER JOIN {$this->sessBDCuenta}.trade.cliente_historico ch ON ch.idCliente = c.idCliente
	...
	public function getVisitas($post)
	{
		...
		$sql = "
			...
			FROM {$this->sessBDCuenta}.trade.data_ruta r
				INNER JOIN {$this->sessBDCuenta}.trade.data_visita v ON r.idRuta = v.idRuta
	...
	public function getVisitasFotos($post)
	{
		$sql = "
			...
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaModuloFotos vmf ON vmf.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vmf.idVisitaFoto
	...
	public function getVisitasPrecios($post)
	{
		$sql = "
			SELECT v.idVisita, 
				...
				vpd.precio
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaPrecios vp ON vp.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaPreciosDet vpd ON vpd.idVisitaPrecios = vp.idVisitaPrecios
	...
	public function getVisitasEncuestas($post)
	{
		$sql = "
			...
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaEncuesta ve ON ve.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaEncuestaDet ved ON ved.idVisitaEncuesta = ve.idVisitaEncuesta
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = ve.idVisitaFoto
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf2 ON vf2.idVisitaFoto = ved.idVisitaFoto
				...
				LEFT JOIN {$this->sessBDCuenta}.trade.encuesta e ON e.idEncuesta = ep.idEncuesta
				INNER JOIN {$this->sessBDCuenta}.trade.data_ruta r ON r.idRuta = v.idRuta
			WHERE v.idVisita IN( ".$post['ids']." )
			ORDER BY v.idVisita DESC, 
					ved.idVisitaEncuestaDet, 
	...
	public function getVisitasSos($post)
	{
		...
		$sql = "
			SELECT DISTINCT 
				...
				pm.nombre marca
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaSos vsos ON vsos.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaSosDet vsosd ON vsosd.idVisitaSos = vsos.idVisitaSos
				...
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vsos.idVisitaFoto
				INNER JOIN {$this->sessBDCuenta}.trade.data_ruta r ON r.idRuta = v.idRuta
			WHERE v.idVisita IN( ".$post['ids']." )
			ORDER BY v.idVisita DESC, 
					vsos.idCategoria;
	...
	public function getVisitasSod($post)
	{
		...
		$sql = "
			...
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaSod vsod ON vsod.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaSodDetFotos vsoddf ON vsoddf.idVisitaSod = vsod.idVisitaSod
				...
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vsoddf.idVisitaFoto
	...
	public function getVisitasSeguimientoPlan($post)
	{
		...
		$sql = "
			...
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaSeguimientoPlan vsp ON vsp.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaSeguimientoPlanDet vspd ON vspd.idVisitaSeguimientoPlan = vsp.idVisitaSeguimientoPlan
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vsp.idVisitaFoto
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vfdet ON vfdet.idVisitaFoto = vspd.idVisitaFoto
				...
				LEFT JOIN {$this->sessBDCuenta}.trade.seguimientoPlan sp ON sp.idSeguimientoPlan = vsp.idSeguimientoPlan
				INNER JOIN {$this->sessBDCuenta}.trade.data_ruta r ON r.idRuta = v.idRuta
			WHERE v.idVisita IN( ".$post['ids']." )
			ORDER BY v.idVisita DESC, 
					vspd.idVisitaSeguimientoPlanDet;
	...
	public function getVisitasPromociones($post)
	{
		...
		$sql = "
			SELECT DISTINCT 
				...
				vf.fotoUrl
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaPromociones vp ON vp.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaPromocionesDet vpd ON vpd.idVisitaPromociones = vp.idVisitaPromociones
	...
	public function getVisitasDespacho($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vf.fotoUrl
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaDespachos vd ON vd.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaDespachosDet vdd ON vdd.idVisitaDespachos = vd.idVisitaDespachos
				LEFT JOIN master.incidencias i ON i.idIncidencia = vdd.idIncidencia
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vdd.idVisitaFoto
				INNER JOIN {$this->sessBDCuenta}.trade.data_ruta r ON r.idRuta = v.idRuta
			WHERE v.idVisita IN( ".$post['ids']." )
			ORDER BY v.idVisita DESC, 
					vdd.idVisitaDesapachosDet;
	...
	public function getVisitasIniciativaTrad($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vf.fotoUrl
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaIniciativaTrad vit ON vit.idVisita = v.idVisita
	...
	public function getVisitasInteligenciaTrad($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vf.idVisitaFoto
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaInteligenciaTrad vit ON vit.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaInteligenciaTradDet vitd ON vitd.idVisitaInteligenciaTrad = vit.idVisitaInteligenciaTrad
	...
	public function getVisitasOrden($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vo.flagOtro
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaOrden vo ON vo.idVisita = v.idVisita
	...
	public function getVisitasVisibilidadTrad($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vf.idVisitaFoto
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaVisibilidadTrad vvt ON vvt.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaVisibilidadTradDet vvtd ON vvtd.idVisitaVisibilidad = vvt.idVisitaVisibilidad
	...
	public function getVisitasCheckList($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vf.idVisitaFoto
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaProductos vp ON vp.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaProductosDet vpd ON vpd.idVisitaProductos = vp.idVisitaProductos
	...
	public function getVisitasInventario($post)
	{
		$sql = "
			SELECT DISTINCT 
				...
				vid.fecVenc
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaInventario vi ON vi.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaInventarioDet vid ON vid.idVisitaInventario = vi.idVisitaInventario
	...
	public function getVisitasIpp($post)
	{
		$sql = "
			SELECT v.horaIni, 
				...
				vf.fotoUrl
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaIpp vi ON vi.idVisita = v.idVisita
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaIppDet vid ON vid.idVisitaIpp = vi.idVisitaIpp
				LEFT JOIN {$this->sessBDCuenta}.trade.ipp i ON i.idIpp = vi.idIpp
				LEFT JOIN {$this->sessBDCuenta}.trade.ipp_alternativa ia ON ia.idAlternativa = vid.idAlternativa
				LEFT JOIN {$this->sessBDCuenta}.trade.ipp_pregunta ip ON ip.idPregunta = vid.idPregunta
				LEFT JOIN {$this->sessBDCuenta}.trade.ipp_criterio ic ON ic.idCriterio = ip.idCriterio
				LEFT JOIN master.tipoPregunta tp ON tp.idTipoPregunta = ip.idTipoPregunta
				LEFT JOIN {$this->sessBDCuenta}.trade.data_visitaFotos vf ON vf.idVisitaFoto = vi.idVisitaFoto
				INNER JOIN {$this->sessBDCuenta}.trade.data_ruta r ON r.idRuta = v.idRuta
			WHERE v.idVisita IN( ".$post['ids']." )
			ORDER BY v.idVisita DESC, 
					vid.idVisitaIppDet;
	...
	public function getVisitasVisibilidadAuditoriaObligatorio($post)
	{
		$sql = "
			SELECT v.horaIni, 
				...
				vvod.cantidad
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaVisibilidadObligatorio vvo ON vvo.idVisita = v.idVisita
	...
	public function getVisitasVisibilidadAuditoriaIniciativa($post)
	{
		$sql = "
			SELECT v.horaIni, 
				...
				oevi.descripcion observacion
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaVisibilidadIniciativa vvi ON vvi.idVisita = v.idVisita
	...
	public function getVisitasVisibilidadAuditoriaAdicional($post)
	{
		$sql = "
			SELECT v.horaIni, 
				...
				vvad.cant
			FROM {$this->sessBDCuenta}.trade.data_visita v
				INNER JOIN {$this->sessBDCuenta}.trade.data_visitaVisibilidadAdicional vva ON vva.idVisita = v.idVisita
