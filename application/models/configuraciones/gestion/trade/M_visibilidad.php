...
class M_visibilidad extends My_Model
{
	...
	public function registrarElemento($post)
	{
		$insert = [
			...
			$this->model->tablas['tipoElemento']['id'] . 'ibilidad' => "1",
		];
		/*if ($this->session->userdata('idProyecto') != null) {
			$insert["idProyecto"] = $this->session->userdata('idProyecto');
		}*/
		if ($this->session->userdata('idCuenta') != null) {
			$insert["idCuenta"] = $this->session->userdata('idCuenta');
		}
	...
	public function getListasModulacion($post)
	{
		$filtros = '';
		if (!empty($post['id'])) $filtros .= " AND idLista = " . $post['id'];
		
		if (!empty($this->session->userdata('idCuenta'))) $filtros .= " AND idCuenta = " .$this->session->userdata('idCuenta');
		...
		$sql = "
			...
			SELECT 
				...
				, fecFin fecFinal
				, estado
			FROM impactTrade_bd.trade.master_listaElementos
			WHERE 1=1 {$filtros}
			ORDER BY estado DESC ,fecInicio DESC
		";
	...
	{
		$filtro = '';
		if (!empty($post['idCanal'])) $filtro .= ' AND seg.idCanal = ' . $post['idCanal'];
		if (!empty($post['idProyecto'])) $filtro .= ' AND ch.idProyecto = ' . $post['idProyecto'];
		if (!empty($post['idCuenta'])) $filtro .= ' AND ch.idCuenta = ' . $post['idCuenta'];
