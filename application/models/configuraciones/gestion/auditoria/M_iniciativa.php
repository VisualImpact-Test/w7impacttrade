...
class M_iniciativa extends My_Model
{
	...
	public function getElementos($post = 'nulo')
	{
		...
		$sql = "
                ...
                JOIN ".$this->tablas['tipoElemento']['tabla']." t ON t.".$this->tablas['tipoElemento']['id']." = p.idIniciativa
                {$filtros}
			";
	...
	public function registrarElemento($post)
	{
		$insert = [
			...
			'idIniciativa' => trim($post['iniciativa']), 
        ];
	...
	public function guardarMasivoLista($multiDataRefactorizada, $idLista)
	{
		...
		foreach($multiDataRefactorizada as $index => $row){
			...
			$rs = $this->db->get_where($this->model->tablas['listaDet']['tabla'],array('idListVisibilidadIni'=>$idLista,'idElementoVis'=>$row['elemento_lista']))->row_array();
			if(count($rs) >=1){
				$repetidos = true;
			...
			foreach ($multiDataRefactorizada as $value) {
				if (empty($value['id'])) {
					$input[] = [
						...
						'idElementoVis' =>$value['elemento_lista'],
