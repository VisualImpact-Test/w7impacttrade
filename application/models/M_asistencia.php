...
class M_asistencia extends MY_Model{
	...
	public function obtener_usuarios_asistencia($input=array()){
		...
		$sql = "
			...
			SELECT DISTINCT
				...
				FROM 
					ImpactTrade_bd.trade.data_ruta r
					JOIN ImpactTrade_bd.trade.data_visita v ON r.idRuta = v.idRuta 
	...
	public function obtener_asistencias($input=array()){
		...
		$this->CI->aSessTrack[] = [ 'idAccion' => 5, 'tabla' => "ImpactTrade_bd.trade.data_asistencia" ];
		return $result;
	}
	...
	public function obtener_foto($input=array()){
		...
		$sql="select fotoUrl from ImpactTrade_bd.trade.data_asistencia  where idUsuario='".$input['idUsuario']."' 
