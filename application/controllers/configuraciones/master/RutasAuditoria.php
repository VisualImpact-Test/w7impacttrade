...
class RutasAuditoria extends MY_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->model('configuraciones/master/m_rutasAuditoria','model');
	}
	...
	public function index(){
		...
		$config['js']['script'] = array(
			...
			, 'assets/custom/js/configuraciones/master/rutasAuditoria'
		);

		$config['nav']['menu_active']='112';
		$config['data']['icon'] = 'fa fa-clock';
		$config['data']['title'] = 'Rutas Auditoria';
		$config['data']['message'] = '';
		$config['view'] = 'modulos/configuraciones/master/rutasAuditoria/index';
	...
	public function filtrar(){
		...
		if ( !empty($ruta) ) {
			...
			$array = array();
			$array['ruta'] = $ruta;			
			$html .= $this->load->view("modulos/configuraciones/master/rutasAuditoria/lista", $array, true);
	...
	public function form_nueva_ruta(){
		...
		$html = $this->load->view("modulos/configuraciones/master/rutasAuditoria/nueva_ruta", $array, true);
...
//print_r($post);
	...
	public function form_add_clientes(){
		...
		$html = $this->load->view("modulos/configuraciones/master/rutasAuditoria/agregar_clientes", $array, true);
		$result['data'] = $html;
		$result['result'] = 1;
		echo json_encode($result);
	}
	...
	public function form_editar_rutas(){
		...
		$result['data']=$this->load->view("modulos/configuraciones/master/rutasAuditoria/editar_rutas", $arreglo, true); 
		echo json_encode($result);
	}
	...
	public function form_editar_usuarios_rutas_tradicional(){
		...
		$result['data']=$this->load->view("modulos/configuraciones/master/rutasAuditoria/editar_usuarios_rutas_tradicional", $arreglo, true); 
		echo json_encode($result);
	}
	...
	public function form_eliminar_rutas(){
		...
		$result['data']=$this->load->view("modulos/configuraciones/master/rutasAuditoria/eliminar_rutas", $arreglo, true); 
		echo json_encode($result);
	}
	...
	public function cargaMasivaRuta(){
		...
		$html='';
		$array=array();
		$html .= $this->load->view("modulos/configuraciones/master/rutasAuditoria/cargaMasivaRutas", $array, true);
		//Result
		$result['result']=1;
	...
	public function clonarRuta(){
		...
		$html .= $this->load->view("modulos/configuraciones/master/rutasAuditoria/clonarRutas", $array, true);
		//Result
		$result['result']=1;
	...
	public function importarRuta(){
		...
		foreach($data_ruta_visitas as $row){
			...
			$clientes[$row['idCliente']]['idVisitaProg'] = $row['idVisitaProg'];
			$clientes[$row['idCliente']]['razonSocial'] = $row['razonSocial'];
			$clientes[$row['idCliente']]['fecha'] = $row['fecIni'];
			//$dias[$row['idCliente']][$row['idDia']]['dia'] = $row['idDia'];
		}
		...
		foreach($clientes as $row => $value){
			...
			$html.='{ idCliente:'.$row.', fecha: "'.$value['fecha'].'" },';
		}
		
		$array['visitas'] = $html;
		
		$html= $this->load->view("modulos/configuraciones/master/rutasAuditoria/rutaImportada", $array, true);
		//Result
		$result['result']=1;
		$result['msg']['title'] = 'CLONAR RUTA';
		$result['data']['html'] = $html;	
		...
		if(count($array_cliente)){
			foreach( $array_cliente as $row ){
				...
				$insert_gtm = array(
				  ...
				  , 'fecIni' => $array_fecha[$row]
				  , 'idUsuario' => $idGtm
				  , 'fecFin' => $array_fecha[$row]
				);
				...
				$fecha = isset($array_fecha[$idCliente])?$array_fecha[$idCliente]:'';
	...
	public function registrarMasivo(){
		...
		foreach($data['clientes'] as $row){
			...
			$array_fecha[$row[0]][$row[2]]=$row[3];
		}
		
		foreach($array_ruta as $row => $value){
			$insert_ruta = array(
				'nombreRuta' => $value
				,'fecIni' => getActualDateTime()
				,'numClientes'=>0
			...
			foreach($array_cliente[$row] as $row_c => $value_c){
				$fecha = isset($array_fecha[$row][$value_c])?$array_fecha[$row][$value_c]:'';
				$insert_gtm = array(
					'idRutaProg' => $idRutaProg
				  , 'fecIni' => $fecha
