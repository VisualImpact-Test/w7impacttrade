	...
	<table id="tb-promociones" class="mb-0 table table-bordered text-nowrap" width="100%">
		<thead>
			<tr>
				...
				$rows = $rows * 4;
				?>
				<th class="text-center" rowspan="1" colspan="<?= $rows ?>">PROMOCIONES</th>
			</tr>
			...
			<tr>
				...
				?>
								...
								<!-- <th class="text-center noVis" rowspan="1" >CANTIDAD</th> -->
			...
			?>
				<tr>
								...
								foreach ($elementos[$idCat] as $idEle => $eleme) {
									...
									foreach ($detalle as $row_d) {
										if ($row['idVisita'] == $row_d['idVisita']) {
											if ($row_d['idPromocion'] == $idEle) {
												$res = false;

												$elemento_pertenece = false;
												if (isset($lista)) {
													if (isset($lista[$row['idVisita']])) {
														if (isset($lista[$row['idVisita']][$row_d['idPromocion']])) {
					...
					?>
												...
												<?
												$fotoImg = (isset($row_d['foto']) && !empty($row_d['foto'])) ? foto_controlador('promociones/' . $row_d['foto']) : '';
												if (!empty($fotoImg)) {
													$fotoImg = '<a href="javascript:;" class="lk-foto-1" data-content="img-fotoprincipal-' . $row_d['idVisita'] . '-' . $row_d['idPromocion'] . '">
																				<img class="fotoMiniatura foto" name="img-fotoprincipal-' . $row_d['idVisita'] . '-' . $row_d['idPromocion'] . '" id="img-fotoprincipal-' . $row_d['idVisita'] . '-' . $row_d['idPromocion'] . '" src="' . $fotoImg . '" alt=""></a>';
												}
												?>
