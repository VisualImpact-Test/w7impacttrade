		...
		<div class="mb-3 card">
			...
			<div class="card-body">
				<form id="frm-registrarModulacionMasiva">
					...
					<div class="row">
						<a href="<?=base_url()?>formato/exclusiones/cargaExclusiones.csv" class="btn-outline-primary border-0" target="_blank" style="padding:10px;margin-left:10px;border-radius: 13px;outline: blue;">  Descargar Formato</a>
						
						<center>
						<input id="archivo" type="file" name="archivo" accept=".csv" style="margin-left:10px;">
						
						<button type="button" data-toggle="collapse" class="btn-outline-primary border-0" style="padding:10px;margin-left:10px;border-radius: 13px;outline: blue;" onclick="Visitas.cargaExclusionesExcel();">Cargar Data</button>
						</center>
...
function validar_estado_carga_exclusiones() {
		$.ajax({
			type: "POST",
			dataType: 'JSON',
			url: site_url+'configuraciones/gestion/visitas/estado_carga_exclusiones',
...
setInterval(validar_estado_carga_exclusiones, 10000);
</script>