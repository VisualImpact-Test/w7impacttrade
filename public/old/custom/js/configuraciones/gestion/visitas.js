var Visitas = {
	...
	load: function(){
		...
		$(document).on('change','input[name="tipoGestor"]', function(e){
			var control = $(this);

			if ( control.val()==1) {
				$('.dvDetalleRutas').show(400);
				$('.dvDetalleVisitas').hide(400);
			} else {
				$('.dvDetalleRutas').hide(400);
				$('.dvDetalleVisitas').show(400);
			}
		});
		...
		$(document).on('click','#btn-rutaNuevo', function(e){
			...
			var data = Fn.formSerializeObject('frm-rutasVisitas');
