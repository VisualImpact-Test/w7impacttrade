		...
		$(document).on('change', '#cbx-live-formato', function(){
			...
			if( $.inArray(grupoCanal, [1,4]) != -1 ){
				if( formato == 1 ){
					...
					$('#cbx-live-distribuidora').val('').change();
					...
					$('#cbx-live-distribuidoraSuc').val('').change();
			...
			$('.formato-live select:visible').attr('patron', 'requerido');
		});
					...
					if( a['result'] != 2 ){
						if( Fn.obj_count(a['data']['region']) == 0 ){
							...
							Fn.showModal({
								...
								'show': true,
								'title': 'Alerta',
								'frm': Fn.message({ 'type': 2, 'message': 'No se encontraron PDV con los datos seleccionados' }),
...
*/
		...
		$(document).on('click', '.page-live-next', function(e){
			...
			if(
				...
				var message = Fn.message({ 'type': 2, 'message': 'Debe elegir un Sub Canal' });
				Fn.showModal({
						'id': ++modalId,
						'show': true,
						'title': 'Alerta',
			...
			$.when( Fn.validateForm({ 'id': frm }) ).then(function(a){
				if( a ){
...
/*
			...
			if( $('#txt-live-fecIni').val().length == 0 ){
				...
				message = Fn.message({ 'type': 2, 'message': 'Debe elegir un Sub Canal' });
				error++;
			}
			else if(
				$('#cbx-live-distribuidora').is(':visible') &&
				...
				$('#cbx-live-distribuidoraSuc').is(':visible') &&
				...
				$('#cbx-live-pdv').is(':visible') &&
				(
					$('#cbx-live-pdv').val() == null ||
