var Archivos = {
    ...
    secciones: ['Archivos','Carpetas','Permisos'],
    tabSeleccionado: '',
    ...
    load: function () {
        ...
        $(".card-body > ul > li > a").click(function (e) {
            ...
            if(indiceSeccion == 0){
                $('.btn-New').hide();
                $('.btn-NewArchivo').show();
            }else if(indiceSeccion == 1 || indiceSeccion == 2){
                $('.btn-New').show();
                $('.btn-NewArchivo').hide();
            }
    ...
    guardarArchivo: function (modalId) {
        ...
        $.when(Fn.ajaxFormData(config)).then(function (b) {
            ++modalId;
            if (b.result == 1) {
                var fn = 'Fn.showModal({ id:' + (modalId - 1) + ',show:false });Fn.closeModals('+modalId+'); Fn.showModal({ id:' + modalId + ',show:false });$(".btn-Consultar").click();';
            }
            else {
                var fn = 'Fn.showModal({ id:' + modalId + ',show:false });';
            }
