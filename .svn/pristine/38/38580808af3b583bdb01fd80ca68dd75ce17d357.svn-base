<div class="card-datatable">
                    <table class="mb-0 table table-bordered text-nowrap w-100 ">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr data-fechaModificacion="<?= !empty($value['fechaModificacion']) ? date_change_format($value['fechaModificacion']) : '-'?>"  data-id="<?= $value[$this->model->tablas['responsable']['id']] ?>" data-estado="<?= $value['estado'] ?>" data-usuario ="<?= !empty($value['usuario']) ? $value['usuario'] : '-' ?>" data-fechaReg =" <?=  date_change_format($value['fechaReg'] )?>"  data-horaReg =" <?=  $value['horaReg'] ?>">
                                    <td></td>
                                    <td></td>
                                    <td>
                                        <div>
                                    ...
                                    <td><?= !empty($value['tipoResponsable']) ? $value['tipoResponsable'] : '-' ?></td>
                                    <td><?= !empty($value['nombres']) ? $value['nombres'] : '-' ?></td>
                                    <td><?= !empty($value['apellidos']) ? $value['apellidos'] : '-' ?></td>
