...
$class = "modalNew";
?>
<form id="formNew" class="frmContenedorselect2">
    <div class='form-row'>
            ...
            <div class="proyecto_sl">
                <select id='proyecto' name='proyecto' class='form-control form-control-sm my_select2'>
                    ...
                    <?php foreach ($proyectos as $idProyecto => $proyecto) { ?>
                        <option value='<?= $proyecto['idProyecto'] ?>'  <?=$this->sessIdProyecto == $proyecto['idProyecto'] ? ' selected ': ''?>><?= $proyecto['nombre'] ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class='col-xs-6 col-sm-6 col-md-6 col-lg-6 mb-2'>
            <label for='grupoCanal'>Grupo Canal</label><br>
                ...
                <select id='grupoCanal' name='grupoCanal' class='form-control form-control-sm my_select2 grupoCanal_cliente'>
                    <option value=''>-- Seleccionar --</option>
                    <? if(!empty($grupocanal)){ ?>
                    <?php foreach ($grupocanal as $idgrupocanal => $gc) { ?>
                        <option value='<?= $gc['idGrupoCanal']?>'><?= $gc['nombre'] ?></option>
                    <?php } ?>
                    <? } ?>
                </select>
            </div>
