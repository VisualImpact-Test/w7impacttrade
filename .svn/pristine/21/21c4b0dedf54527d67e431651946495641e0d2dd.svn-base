...
<form id="formUpdate">
    <div class='form-row'>
    <input class="d-none" type="text" name="idx" value="<?= $data[$this->model->tablas['confTipoCliente']['id']] ?>">
    <div class='col-xs-12 col-sm-12 col-md-12 col-lg-12 mb-2'>
        ...
        <label for='sl_tipoAuditoria'>Tipo Auditoría</label><br>    
            <select class="my_select2_modal form-control" name="sl_tipoAuditoria" id="sl_tipoAuditoria">
                <?=htmlSelectOptionArray($tiposAuditoria,$data['idExtAudTipo'])?>
            </select>
        </div>
        <div class='col-xs-6 col-sm-6 col-md-6 col-lg-6 mb-2'>
                ...
                <table class="mb-0 table table-bordered no-footer w-100 text-nowrap">
                    <thead class="thead-light"> 
                        <tr>
                            ...
                            <th class="text-center matExtVar" style="width: 80%;">MATERIAL (<?=count($dataDet)?>)</th>
                            <!-- <th class="text-center " style="width: 80%;">ELIMINAR</th> -->

                        </tr>
                    </thead>
                    <tbody id="tbodyMateriales">
                            <?foreach($dataDet as $i => $v):?>
                                <tr>
                                    <td class="text-center"><?=generarCorrelativo($v['idExtAudMat'],6)?><input type="hidden" name="idMaterial" value="<?=$v['idExtAudMat']?>"></td>
                                    <td><?=$v['material']?><input type="hidden" name="material" value="<?=$v['material']?>"></td>
                                    <!-- <td class="text-center"><btn class="btn btn-danger fas fa-trash  btn-borrarfila" data-idMat = "<?=$v['idExtAudMat']?>"></btn></td> -->
                                </tr>
                            <?endforeach;?>
                    </tbody>
                </table>
