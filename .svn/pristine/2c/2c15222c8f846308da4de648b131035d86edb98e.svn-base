		...
		<div class="col-md-12">
			<div class="mb-3 mt-3 card">
				<div class="card-header">
					EDICIÓN DEL MÓDULO DE VISIBILIDAD AUDITORIA INICIATIVA
				</div>
				<div class="card-body">
					<div class="tab-content">
						<div class="table-responsive">
							<table id="tb-visibilidadIniciativa" class="mb-0 table table-bordered table-sm text-nowrap">
								<thead>
									<tr>
								...
								<tbody class="tb-visibilidadIniciativa">
									<? $ixt=1; ?>
									<? foreach ($listaElementoVisibilidad as $klev => $elementos): ?>
										...
										?>
										<tr class="tr-visibilidadIniciativa" data-elementoVisibilidad="<?=$klev?>" data-visitaVisibilidad="<?=$visitaVisibilidad?>" data-visitaVisibilidadDet="<?=$visitaVisibilidadDet?>">
											...
											<td class="text-center">
												<? $visitaObservaciones = ( isset($listaVisitas[$klev]['idObservacion']) && !empty($listaVisitas[$klev]['idObservacion']) ) ? $listaVisitas[$klev]['idObservacion']:'';?>
												<select class="form-control slWidth" name="observacion-<?=$klev?>" id="observacion-<?=$klev?>">
													<option value="">--- Observaciones ---</option>
													<? foreach ($listaObservaciones as $klvo => $observaciones): ?>
														<? $checkedObservaciones = $klvo==$visitaObservaciones ? 'selected':'';?>
														<option value="<?=$observaciones['idObservacion']?>" <?=$checkedObservaciones?>><?=$observaciones['descripcion'];?></option>
													<? endforeach ?>
												</select>
											</td>
											<td class="text-center">
